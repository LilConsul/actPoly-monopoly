...
export const GameComponent = () => {
  ...
  // Rotates the text 90° counterclockwise for every cell
  const renderTileLabel = (tile) => {
    ...
    return (
      <span className="text-xs font-medium text-center truncate transform -rotate-90">
        {label}
      </span>
    );
  };

  // ─── BOARD RENDERERS WITH UPDATED DIMENSIONS AND ROTATED TEXT ─────────────

  // Top row: horizontal cells (with corners at indices 20 and 30)
  const renderTopRow = () => {
    const cells = [];
    for (let i = 20; i <= 30; i++) {
      ...
      const cellClasses = isCorner
        ? 'border border-black flex items-center justify-center relative bg-white cursor-pointer overflow-hidden w-20 h-20 p-2'
        : 'border border-black flex items-center justify-center relative bg-white cursor-pointer overflow-hidden w-16 h-20 p-1';
      cells.push(
        <div
          key={`top-${i}`}
  ...
  // Bottom row: horizontal cells (with corners at indices 0 and 10)
  ...
  // Left column: vertical cells (non-corner) using fixed dimensions
  const renderLeftColumn = () => {
    ...
    for (let i = 19; i >= 11; i--) {
      ...
      const tileStyle = getTileStyle(tile);
      const cellClasses =
        'border border-black flex items-center justify-center relative bg-white cursor-pointer overflow-hidden w-20 h-16 p-1';
      cells.push(
        <div
          key={`left-${i}`}
          className={cellClasses}
  ...
  // Right column: vertical cells (non-corner) using fixed dimensions
    ...
    <div className="container mx-auto p-6 max-w-5xl">
      <Card className="mb-6">
        ...
        <CardContent>
          ...
          {/* Board container now uses a width of 736px */}
          <div className="border-2 border-black w-[736px] mx-auto">
            {renderTopRow()}
            <div className="flex">
              {renderLeftColumn()}
